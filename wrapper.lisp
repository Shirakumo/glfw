...
(defmethod list-video-modes ((monitor monitor))
  (cffi:with-foreign-object (count :int)
    (let ((array (ignore-errors (glfw get-video-modes (pointer monitor) count))))
      (when array
...
(defmethod shared-initialize :after ((window window) slots &rest args &key initialize-context monitor share
  ...
  (when initialize-context
    ...
    (when (eql T (width window))
      (let ((mode (video-mode (or monitor (primary-monitor)))))
...
(defun find-window-monitor (window)
  (let* ((monitors (list-monitors))
         (best (first monitors)))
    (destructuring-bind (ww wh) (size window)
      (destructuring-bind (&optional wx wy) (ignore-errors (location window))
        ;; KLUDGE: On Wayland you cannot get the window position (lol) so we can't get the current monitor.
        (if wx
            (flet ((monitor-area (monitor)
                     (destructuring-bind (mx my) (location monitor)
                       (let* ((mode (video-mode monitor))
                              (x- (max wx mx))
                              (y- (max wy my))
                              (x+ (min (+ wx ww) (+ mx (first mode))))
                              (y+ (min (+ wy wh) (+ my (second mode)))))
                         (* (- x+ x-) (- y+ y-))))))
              (dolist (monitor (rest monitors) best)
                (when (< (monitor-area best) (monitor-area monitor))
                  (setf best monitor))))
            (or (ptr-object (glfw get-window-monitor (pointer window)))
                (primary-monitor)))))))

(defmethod monitor ((window window))
  (or (ptr-object (glfw get-window-monitor (pointer window)))
      (find-window-monitor window)))
...
(defmethod (setf monitor) ((none null) (window window))
  (setf (monitor window) (list NIL)))
...
(defmacro with-game-loop ((window dt &rest initargs) &body body)
  (let ((last-time (gensym "LAST-TIME"))
        (new-time (gensym "NEW-TIME"))
        (time-resolution (gensym "TIME-RESOLUTION")))
    `(progn
       (init)
       (unwind-protect
            (let ((,window (make-instance 'window ,@initargs))
                  (,last-time (timestamp))
                  (,time-resolution (/ 1.0d0 (timestamp-resolution))))
              (declare (type (unsigned-byte 64) ,last-time))
              (loop until (should-close-p ,window)
                    do (poll-events)
                       (let* ((,window ,window)
                              (,new-time (timestamp))
